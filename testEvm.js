...
async function signOrder(order, chainId, target, walletClient) {
    const orderData = buildOrderData(chainId, target, order);
    console.log(orderData.message);
    return await walletClient.signTypedData(orderData);
}


// Configuration
const RESOLVER_ADDRESS = '0x9873C1D68CaAF37599C8C6C14F07AA1A1D12516E' // Resolver contract address
...
const ESCROW_FACTORY_ADDRESS = '0xC76BdddF3A157eF572D7367Ebfa51a3DF5Ef6184' // Replace with EscrowFactory address
...
const salt = saltPrefix | extensionHashSuffix;
  // const salt = 123n << 160n // upper 96 bits set to 123, lower 160 bits zero
    ...
    const ext = (await buildOrder(order, {postInteraction:ESCROW_FACTORY_ADDRESS,}, hashlock))
    order.salt = ext.salt
...
function buildOrder (
    ...
    console.log('hahsy', hashlock.slice(2))
    extension = "0x0000010f0000004a0000004a0000004a0000004a000000250000000000000000"+ESCROW_FACTORY_ADDRESS.slice(2)+"00000000000000688c9d28000078000000"+ESCROW_FACTORY_ADDRESS.slice(2)+"00000000000000688c9d28000078000000"+ESCROW_FACTORY_ADDRESS.slice(2)+"00000000502ea634d146790ca4a6000008"+hashlock.slice(2)+""
